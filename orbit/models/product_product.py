...
class ProductTemplate(models.Model):
    ...
    def _onchange_markup_percentage(self):
        ...
        for product in self:
            if product.standard_price > 0:
                if product.product.markup_percentage:
    ...
    def _compute_sale_type_qty(self, sale_type):
        """Méthode générique pour calculer les quantités par type de vente"""
        product_ids = self.ids
        domain = [
            ('product_id', 'in', product_ids),
            ('order_id.state', 'in', ['sale', 'done']),  # États standard Odoo
            ('order_id.type_sale', '=', sale_type)
        ]
        
        # Requête optimisée avec read_group
        res = self.env['sale.order.line'].read_group(
            domain=domain,
            fields=['product_id', 'product_uom_qty', 'qty_delivered'],
            groupby=['product_id']
        )
        
        # Création d'un dictionnaire {product_id: values}
        qty_dict = {
            x['product_id'][0]: {
                'ordered': x['product_uom_qty'],
                'delivered': x['qty_delivered']
            } for x in res
        }
        
        for product in self:
            values = qty_dict.get(product.id, {})
            product_qty = values.get('ordered', 0) - values.get('delivered', 0)
            return product_qty
    
    def _compute_preordered_qty(self):
        for product in self:
            product.preordered_qty = self._compute_sale_type_qty('preorder')

    
    def _compute_creditorder_qty(self):
        for product in self:
            product.creditorder_qty = self._compute_sale_type_qty('creditorder')

   
    def _compute_ordered_qty(self):
        for product in self:
            product.ordered_qty = self._compute_sale_type_qty('order')
